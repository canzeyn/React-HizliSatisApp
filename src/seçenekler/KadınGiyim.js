...
const KadınGiyim = () => {
  ...
  const kadınGiyim = useSelector((state) => state.urunler.ürünler.kadın_giyim_Ürünler );
...
<div className="flex justify-center mt-5 mb-5">
    ...
    <div className=" grid grid-rows-auto grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mt-2  mr-2 ml-2">
      {kadınGiyim.filter((urun) => urun.productName.toLowerCase().includes(searchTerm.toLowerCase())).map((urun) => (
